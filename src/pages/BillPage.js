...
async function getGarageDetails() {
  ...
  const res = await fetch(
    `http://optimus-internal.suprrfix.com/api/v1/garage/${currentGarage}/details`,
    {
      cache: "no-store",
      headers: {
...
export default function BillPage() {
  ...
  const handleCheckout = async () => {
    ...
    const response = await fetch("http://optimus-internal.suprrfix.com/api/v1/create/check_out", {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
